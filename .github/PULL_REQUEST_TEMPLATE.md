## Summary

Brief description of what this PR does.

## Type of Change

- [ ] New agent
- [ ] New skill
- [ ] New hook or rule
- [ ] Bug fix
- [ ] Documentation improvement
- [ ] Other:

## Changes

-
-
-

## Checklist

- [ ] I've tested this in a Codex task
- [ ] New subagents have a `.codex/agents/<name>.toml` definition and documented role boundaries
- [ ] New skills use the subdirectory format (`.agents/skills/<name>/SKILL.md`)
- [ ] New path rules use the closest applicable nested `AGENTS.md`
- [ ] Reference docs are updated (agent-roster, skills-reference, hooks-reference, rules-reference)
- [ ] Hooks use `grep -E` (POSIX) and fail gracefully without jq/python
- [ ] No hardcoded paths or platform-specific assumptions
