...
## 1. Canonical serialization and hashing

- Hash exact raw bytes with SHA-256 and render `sha256:<64-lower-hex>`.
...
- Hash a set as SHA-256 over ordered length-delimited records
  `byte_length ":" raw_bytes`. A set contract MUST name its sort key.
- `ABSENT` is a precondition token, not a hash. It requires no filesystem entry
  at the canonical path immediately before create.
- A raw hash mismatch is a conflict even when parsed semantics look equal.
...
readiness:
  ...
  registry_expected_sha256: sha256:<digest>
  record_id: <stable-record-id>
  record_expected_sha256: sha256:<digest>
  candidate_expected_sha256: sha256:<digest>
  readiness_key: <stable-key>
  recorder_receipt_path: <project-relative-path>
  recorder_receipt_expected_sha256: sha256:<digest>
sprint:
  id: <stable-id>
  plan_file: <project-relative-path>
  plan_sha256: sha256:<exact-raw-plan-bytes>
  tracker:
    path: production/sprint-status.yaml
    raw_sha256: sha256:<exact-raw-tracker-bytes>
    ...
    plan_sha256: <same-exact-raw-plan-hash>
...
resume:
  ...
  checkpoint_expected_sha256: sha256:<digest> | NONE
...
```text
kind / stable_id / canonical_path / expected_raw_sha256 / schema_or_format /
...
A proposed new target appears as `kind: target` with `expected_raw_sha256:
...
1. record schema and canonical hash are valid;
2. final verdict is `READY` and evaluation state is `COMPLETE`;
3. registry identity/revision/head and raw hash match the recorder receipt;
...
7. story ID/path/raw hash match the request and current bytes;
...
implemented_against_hash: sha256:<captured-story-source-hash>
current_hash: sha256:<current-control-manifest-hash>
accepted_by: <user-identity>
accepted_at: <RFC-3339-UTC>
provenance_status: ACCEPTED-RISK
```

The story header/source snapshot MUST agree on `implemented_against_hash`; the
current raw manifest MUST equal `current_hash`; and every non-manifest check MUST
pass. Never change captured version/hash/snapshot fields. A new current manifest
hash invalidates the waiver and plan.
...
dependency_sha256: sha256:<current-raw-hash> | MISSING
observed_status: <exact-current-status> | MISSING
accepted_by: <user-identity>
accepted_at: <RFC-3339-UTC>
expires_when: <dependency path/hash/status or governing source changes>
reason: <specific-bounded-risk>
```

The dependency MUST be explicitly soft. The waiver MUST match freshly observed
identity/path/hash/state and is invalid after any change. A hard dependency has
...
the final pre-write CAS. It does prevent reuse or resume unless the resume
...
The optional engine reviewer receives immutable bytes or exact hash-bound
...
canonical_plan_sha256: sha256:<digest>
```
...
expected_preimage(hash|ABSENT) / output_kind(DETERMINISTIC|RUNTIME_DERIVED) /
candidate_sha256-or-derivation_contract_sha256 / candidate_or_output_byte_bounds /
semantic_change / AC_IDs / Test_IDs / source_binding_hashes / write_condition /
rollback_classification
```

Sort targets by canonical path after uniqueness checks. Deterministic candidate
bytes are prepared before approval and their hash cannot be `TBD`. Runtime-derived
...
canonical derivation contract, exact inputs/schema/limits, expected preimage, and
post-write validation; its final raw hash is recorded only after generation. A
runtime output cannot expand the path set or semantic scope. The plan hash is
computed from the canonical plan excluding its own hash. `plan_id` is the first
20 plan-hash hex characters prefixed `DSP-`.

Authorization records user/task identity, RFC 3339 UTC time, exact `plan_hash`,
ordered target-contract-set hash, owner-set hash, source-snapshot hash, and the
...
evidence: <ordered exact path/hash/locator rows>
owner: architecture-decision-owner
required_action: <one bounded decision question>
resolution_condition: <Accepted ADR ID/path/hash/locator covering the change>
```

Finding identity hashes schema version, story ID, classification, change key,
normalized observation key, and resolution owner/condition. It excludes prose,
line numbers, timestamps, and source byte hashes so the same unresolved decision
...
hashes, configured engine/version, planned file types and exact read-only paths,
relevant ADR/control/source hashes, questions, no-write rule, and response schema.

`cgs.dev-story-engine-review-result/v1` binds the packet hash and contains ordered
...
`lifecycle_owner`, `lifecycle_recorder`, `plan_file`, `plan_sha256`,
`plan_revision`, `story_set_hash`, `updated_at`, `goal`, `start_date`, `end_date`,
...
cgs.sprint-tracker/v2`. The consumer independently hashes exact tracker bytes;
that raw SHA-256 is a request/manifest/CAS binding and MUST NOT be read from or
written as a tracker field.

The selected plan path and exact raw bytes MUST match tracker `plan_file` and
`plan_sha256`. Freeze `plan_revision` and `story_set_hash`; lifecycle transitions
do not own or recompute them. Validate typed capacity, its receipt/raw hash and
...
expected_tracker_sha256: sha256:<external-raw-byte-hash>
expected_tracker_revision: <positive-integer>
expected_plan_revision: sha256:<unchanged-digest>
expected_story_set_hash: sha256:<unchanged-digest>
...
story_sha256: sha256:<immutable-raw-byte-hash>
from_status: ready_for_dev | in_progress
to_status: in_progress | in_review
provenance: <plan/readiness/result-set/owner bindings>
```

The proposal preserves tracker `plan_file`, `plan_sha256`, `plan_revision`,
`story_set_hash`, sprint identity, capacity, QA-plan, all unowned story fields,
and every other row. `IN_PROGRESS` derives from the approved plan, current tracker
preimage, immutable story/readiness admission, transaction ID, and one timestamp.
`IN_REVIEW` additionally binds verified actual target hashes and canonical
execution records.
...
proposal hash, lifecycle owner/recorder identities, exact tracker pre/post raw
hashes and byte counts, prior/new revisions, event/transaction IDs, requested
field-owner set, observed row transition/provenance, CAS outcome `COMMITTED`,
read-back `VERIFIED`, immutable story hash, unchanged planning tuple, and complete
...
runner:
  ...
  executable_sha256: sha256:<digest> | NOT_APPLICABLE_WITH_REASON
source_identity: <ordered path/hash set>
build_identity: <exact build/candidate hash | NOT_APPLICABLE_WITH_REASON>
expected: <exact exit/result/assertion contract>
raw_log_path: <implementation-owner target path>
raw_log_expected_preimage: sha256:<digest> | ABSENT
raw_log_derivation_contract_sha256: sha256:<digest>
...
byte count/hash, assertion observations, and normalized result. The result-set
hash is computed over Test-ID-sorted canonical execution records.
...
observer identity, exact build, procedure, time, result, and raw asset hash.
...
complete hashable log, and met the expected contract. `SKIPPED`, `UNKNOWN`,
...
```text
checkpoint ID/hash / created-at / result PARTIAL / request+plan+authorization /
...
planned+actual write sets / per-path baseline+candidate+current hashes /
...
Checkpoint ID hashes plan, transaction, failure classification, actual-write set,
observed tracker/receipt state, and safe resume point; it excludes timestamp and prose. The
checkpoint hash covers complete canonical bytes.

Resume requires an exact request/checkpoint/hash and the same plan/authorization,
...
```text
schema/outcome / workflow+request+plan+authorization hashes / story identity /
readiness consumption / source/dependency/architecture/engine-review results /
owner domains / planned+actual writes and hashes / lifecycle transaction evidence /
...
next owner/action / generated-at / canonical result hash
```
...
- `BLOCKED`: a contract, evidence, dependency, architecture, CAS, or authorization
