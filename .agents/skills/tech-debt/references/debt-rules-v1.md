...
- schema, project ID, canonical project root, target commit/ref plus dirty-state
  receipt, manifest ID, generated-at timestamp, and normalized manifest hash;
- explicit included path entries with type, language, byte size, and SHA-256;
- explicit exclusion entries with classification, reason, classifier evidence,
  path, size, and hash when readable;
- requested checks, exact rule IDs, analyzer adapter IDs/versions, rulepack/config
  paths and hashes, and declared capabilities;
...
preserve aggregate/result hashes and mark remaining rows `NOT_ENUMERATED`; the
...
rulepack_id/version/hash    exact packaged rule definition
...
require an explicit fingerprint migration/alias proposal; never silently split or
merge identities.

Analyzer adapter entries define adapter ID/version/hash, executable identity,
...
receipt_id                  UUIDv4
schema
project_id / target_id / scope_manifest_hash
adapter_id / adapter_version / adapter_hash
executable / executable_version / executable_hash when available
rule_id / rule_version / rulepack_id / rulepack_version / rulepack_hash
config_hash
exact redacted argv / canonical cwd
included_subset_manifest_hash
...
raw_result_hash / normalized_result_hash / redacted_log_hash
...
```text
candidate_id                TDC- plus first 20 lowercase hex of fingerprint, or
                            TDC-UNVERIFIED-<receipt UUID> when no fingerprint
...
source byte hash / structural evidence locator
evidence excerpt hash; redacted bounded excerpt only when safe
analyzer receipt IDs and hashes
defect_class / category suggestion or UNKNOWN
why_debt / why_intentional
confidence and basis
owner_triage                 REQUIRED | SELECTED | REJECTED | DEFERRED
fingerprint_version / fingerprint or UNKNOWN
...
A `cgs.tech-debt-manual-candidate/v1` contains schema, project/target identity,
canonical affected path entries and source hashes, stable symbol or `<file>`,
stable defect class, concise description, evidence references/hashes, category or
`UNKNOWN`, owner or `UNASSIGNED`, why the item requires owner triage, optional
impact/frequency/effort values with fixed-scale evidence, creator, created-at, and
normalized record SHA-256. It must not contain an asserted lifecycle transition,
accepted-risk/debt decision, product priority, or schedule. Unknown required
identity/evidence fields are an input error; allowed unknown classification/
estimate values stay explicit.

## 6. Stable fingerprint v2

Canonicalize fields with Unicode NFC, LF line endings, `/` path separators, and
repository-canonical casing. Encode each as UTF-8 `decimal-byte-length:bytes`,
concatenate in the listed order with no delimiter, and compute lowercase SHA-256:
...
Exclude line/column, byte offsets, source/revision/hash, observed metric value,
threshold, timestamp, analyzer/rulepack version, diagnostic wording, excerpt
hash, confidence, severity, score, owner, and lifecycle state.

The scanner records all excluded evidence separately. If a required field is
ambiguous or an analyzer cannot provide a stable structural identity, fingerprint
is `UNKNOWN`; do not synthesize one.

Aliases use `cgs.tech-debt-fingerprint-alias/v1`: old/new fingerprints, stable
debt UUID, VCS move/copy evidence identity or explicit user selection, exact
source identities, reason, proposer, and proposal hash. The analyzer may propose
...
fingerprint_version         td-fp-v2
event_schema                cgs.tech-debt-event/v3
```

Each `COMMIT` marker contains `record_type: COMMIT`, `commit_id` UUIDv4,
monotonic `revision` (previous + 1), `event_count`, `last_event_id/hash` or
`NONE`/`SHA256_EMPTY`, previous commit ID/hash, hash of the complete byte prefix
through the preceding event (or genesis), committed timestamp, authorized
recorder transaction/receipt identity, and commit-record hash. The current
register identity is the full-file SHA-256 plus the latest commit tuple. A commit
with no new events is permitted only for genesis revision 0; later empty commits
are invalid.
...
source_target_id / source_hash / evidence_refs
fingerprint / alias fields when applicable
previous_global_event_id/hash
previous_debt_event_id/hash or NONE
payload / payload_hash
event_hash                  hash of canonical event excluding event_hash
```
...
|---|---|
| `CREATED` | primary fingerprint, description, defect class, category/owner or UNKNOWN/UNASSIGNED, initial evidence; creates `OPEN` |
| `OBSERVED` | new source identity/evidence hash and observation reason; status unchanged |
...
| `RESOLVED` | authorized resolver, resolution reason, changed artifact/test evidence hashes, verification reference; status `RESOLVED` |
| `SUPERSEDED` | authorized owner, reason, successor debt UUID(s); status `SUPERSEDED` |
| `FINGERPRINT_ALIAS` | alias record and move/user evidence; status unchanged |
| `ESTIMATE_UPDATED` | impact/frequency/effort values or UNKNOWN, scale/version, evidence; status unchanged |
| `PRIORITY_SELECTED` | producer/user decision, reason, source advisory view hash; status unchanged |
...
`SUPERSEDED` is terminal. `OBSERVED`, alias, estimate, priority, and schedule
events do not change lifecycle state. Any undeclared transition, missing reason/
owner/authority/timestamp/hash, duplicate fingerprint owner, event/UUID collision,
broken chain, or unrecognized event is `REGISTER_ERROR`. Unknown legacy status
remains `UNKNOWN` and requires migration/triage; it is not a v3 lifecycle value.

The materialized item view contains stable debt ID, primary/alias fingerprints,
...
- A valid recognized v1/v2 register produces `REGISTER_ERROR` subtype
  ...
  SHA-256 and either preserve those bytes in an immutable appendix or point to an
  immutable snapshot. It must define deterministic ID/fingerprint/status mappings
  and all `UNKNOWN` fields. Migration needs a separate recorder transaction and
  new exact authorization.
- An unrecognized schema, malformed syntax, hash/chain mismatch, invalid
...
Replay the valid register into one index of primary fingerprints and aliases.
Exactly one `CREATED` event owns a fingerprint. For each candidate:

1. No match: classify `NEW`; after owner selection propose one `CREATED` event.
2. Match and same source identity plus evidence hash as the last observation:
...
3. Match and materially new evidence at a new source identity: classify `MATCHED`;
   ...
   from `td-fp-v2` may be evidence for an observation.
...
Multiple candidates with one fingerprint in one run collapse to one candidate
...
latest valid `ESTIMATE_UPDATED` event and evidence hash. Exact rational comparison
...
register ID/hash/revision/head, selection filter, generated-at timestamp, row
inputs/evidence/formula/tie-break trace, gaps, and normalized view SHA-256.
...
A baseline event must exist in the current global chain. A baseline register hash
must identify an immutable ancestor whose head event/hash can be proven. A sibling,
...
schema / operation
base_content_hash or ABSENT
base_revision / base_last_event_id/hash
ordered exact event bytes / reserved event and debt UUIDs
dedup index hash / selected candidate or decision hashes
authority references required by event types
proposed revision / last event ID/hash / content hash
append_only_proof
stale_key
...
these are included in its hash. Any changed byte, UUID, time, event order, dedup
result, base, authority reference, or proposed hash is a new proposal requiring
...
1. accept one exact proposal hash plus separate current authorization identifying
   operation, register path, event IDs, final hash, authorizer, and expiry;
2. acquire an implementation-defined exclusive lock or use a filesystem/database
   primitive that supports atomic compare-and-set;
3. reread and validate the current register schema, chains, lifecycle,
   fingerprint ownership, and current UUID uniqueness;
4. return `ALREADY_APPLIED` without writing only when the current full-file hash
   equals the proposed final hash and every exact event/commit ID and hash already
   exists; a superficially equivalent event is not silently adopted;
...
6. compare the exact base tuple `(path, register_id, content_hash, revision,
   last_event_id, last_event_hash)` or confirmed absence, and return
...
7. revalidate proposal bytes, authority, reserved UUID uniqueness, lifecycle,
   predecessors, and event/payload/commit/final hashes against that exact base;
...
9. reread and verify exact final hash, revision/head, prior-byte prefix/order,
   chains, lifecycle, ownership, and uniqueness; and
10. emit an immutable recorder receipt containing base/final identity, lock/CAS/
    atomicity evidence, authorization identity/hash, event IDs, timings, result,
    and receipt hash.
...
project/target/scope identity and hashes
register path/presence/schema/id/hash/revision/head
input records and hashes
coverage ledger / fixed-limit use / exclusions / gaps
analyzer receipts and hashes
candidates / fingerprints / dedup classifications
materialized lifecycle view hash
advisory view or report/baseline as applicable
change proposal or NONE
before/after mutation snapshot hashes and changed paths
...
The stale key includes project target identity, scope/input hashes, register base
identity, analyzer/rulepack/config hashes, and baseline identity. Any change makes
