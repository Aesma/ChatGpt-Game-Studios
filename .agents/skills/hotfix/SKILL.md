---
name: hotfix
description: "Builds an isolated, hash-bound hotfix candidate with deterministic QA scope, bounded review convergence, verified rollback, and deployment-receipt observation without merging or deploying."
---
...
$hotfix prepare --action-manifest {path} --action-sha256 {sha256}
$hotfix apply --run-manifest {path} --run-sha256 {sha256}
$hotfix review --review-manifest {path} --review-sha256 {sha256}
$hotfix commit --action-manifest {path} --action-sha256 {sha256}
$hotfix build --action-manifest {path} --action-sha256 {sha256}
$hotfix assess --assessment-manifest {path} --assessment-sha256 {sha256}
$hotfix record-candidate --link-manifest {path} --link-sha256 {sha256}
$hotfix push --action-manifest {path} --action-sha256 {sha256}
$hotfix handoff --handoff-manifest {path} --handoff-sha256 {sha256}
$hotfix observe --observation-manifest {path} --observation-sha256 {sha256}
$hotfix status --run-manifest {path} --run-sha256 {sha256}
~~~

Reject missing/unknown commands, positional bug/branch/environment guesses, unknown or duplicate flags, missing values, unsafe IDs/paths, absolute paths, globs, directory scans, and non-lowercase 64-hex SHA-256 values. There is no merge, tag, deploy, rollback-execution, publish, or send command. A request for one is rejected before reading project state.
...
Unknown or incompatible schemas are blocking. Every artifact records the workflow-contract path/hash and canonicalization version.
...
Every writer owns one exact path. Every final target is absent unless a schema explicitly defines a compare-and-swap projection. Never select latest files, overwrite immutable receipts, or let two roles write one artifact.
...
- `apply` requires exact file-path/preimage authorization.
...
`plan` first validates one exact canonical bug record or complete reproducible description. For a bug, require canonical path/hash, schema, stable Bug ID, severity, status, Repro Case ID, actual/expected result, and evidence references. Emergency planning accepts only policy-authorized severity, normally S1-Critical or S2-Major; this workflow does not change severity or priority.

Record `cgs-hotfix-repo-preflight/v1` with:
- repository root and identity;
- applicable instruction and repository-policy paths/hashes;
...
- inspection timestamp and raw command/output hashes where applicable.
...
- Hotfix/Bug/Repro IDs and canonical bug path/hash;
- repository/base full commit/tree and preflight receipt hash;
- isolated branch/worktree proposal;
- every code/test/local-receipt path, preimage SHA-256 or `ABSENT`, unique writer, and intended change;
- explicitly excluded refactors, feature work, versions, generated noise, and release notes;
- required failure-sensitive regression Test ID/path/source hash;
- candidate build target and exact build/test/QA manifest paths/hashes;
- complete risk inputs and deterministic QA-scope result/hash;
...
- plan expiry/currentness rules and SHA-256.

Only after this read-only plan has exact patch paths/preimages may file writes be authorized. A newly required path, changed intent, new risk input, or preimage mismatch requires a new plan and authorization.

## Phase 2: Prepare only an isolated worktree

`prepare` re-hashes the plan and reruns preflight. Its action manifest binds action/idempotency ID, repository identity, base full commit/tree, exact new branch/ref and worktree path, expected absence, executor, timeout, recovery owner, and receipt destination.
...
`apply` requires WORKTREE_READY, the exact run/plan/preflight hashes, and authorization for each path/preimage/intended change. Re-hash isolated HEAD/tree and every preimage.
...
The patch receipt records every path, pre/post hash, normalized diff hash, test-source hash, writer, timestamps, and actual result. `PATCH_APPLIED` means only the isolated files changed as authorized.

## Phase 4: Stable findings and bounded correction convergence

Every review, QA, rollback, or interface issue uses `cgs-hotfix-finding/v1`. Compute its fingerprint from:
`workflow-contract hash | Hotfix ID | candidate/plan hash | reviewed artifact hash | rule ID | stable subject | expected value | observed value`.

Finding ID is `HF-F-{first-16-hex-of-fingerprint}` and stores the full SHA-256, severity `BLOCKER|MAJOR|MINOR|INFO`, owner, artifact references, remediation condition, first-seen iteration, and current state. Sort by severity, rule ID, subject, then fingerprint. Unchanged bytes reproduce identical IDs and order.
...
Every assigned implementation, review, build, regression, smoke, rollback, or observation task has a `cgs-hotfix-task-receipt/v1` binding Task ID, owner identity/role, exact inputs/hashes, allowed operations, output path, start/deadline/end, status, and result hash.
...
Normalize stable Test/Check IDs, sort bytewise, reject duplicates/conflicts, and compute `qa_scope_sha256`. QA may add checks with a reason but may not remove or downgrade a mapped requirement. A discretionary smoke/targeted/full choice cannot replace this matrix.

## Phase 7: Commit and build as separate actions

`commit` requires the exact patch and review state, expected diff, staged path/blob set, parent full commit/tree, commit message bytes/hash, author/executor, signing policy, receipt path, and explicit commit authorization. Stage only listed paths, create one commit, then verify full commit/tree/parent/message/signature and clean isolated state. COMMIT_CREATED does not mean pushed, built, tested, or ready.

`build` requires an exact action manifest with ordered argv array, cwd, environment allowlist, toolchain/container/config hashes, timeout/output limits, process cleanup, output destinations, and explicit authorization. Never synthesize commands.

The build receipt and `cgs-build-candidate/v1` bind full source commit/tree, candidate/build/artifact IDs and SHA-256, platform/configuration, toolchain, argv, timestamps, runner/job, exit code, complete log hash, QA-plan hash, test-execution-manifest hash, signature/provenance/SBOM when required, and read-back hashes. Any rebuild or changed byte creates a new candidate and stales dependent evidence.

## Phase 8: Consume only versioned interface artifacts

The run manifest contains `cgs-hotfix-interface-bundle/v1`. Each interface entry has producer, schema, exact path/raw SHA-256, candidate/build/artifact/source/platform binding, status mapping, freshness rule, and verifier.
...
| QA scope owner | `cgs-hotfix-qa-scope/v1` | exact scope hash only; selection does not prove execution |
...
| release owner | `cgs.release-action-receipt/v2` | only `DEPLOY`/`production`/`SUCCESS` with exact release/candidate/build/deployment authority and hashes may start observation |
| observation owner | `cgs-hotfix-deployment-observation/v1` | proves only its complete health window and bound deployment |

Unknown schemas, missing verifiers, unsupported versions, unreadable bytes, partial/unpersisted artifacts, or hash/identity mismatch map to PARTIAL, UNKNOWN, STALE, UNAVAILABLE, or INVALID and block dependent results. Never parse conversational text or invoke the producer workflow.

For code review, re-hash the generic envelope and embedded extension plus every
...
path/hash, record `RECORDED|ALREADY_RECORDED`, PERSISTED and gate eligible true,
and bind recorder authority, registry final hash, CAS/atomicity and read-back.
For Team QA, re-hash result/signoff and require matching request/run/scope,
...
- trusted current-production-state receipt with account/project/region/environment, deployment ID, build/artifact/source identities, issuer, query time, and raw response hash;
- exact rollback build/artifact path/hash, signature/provenance, availability, and retention;
...
- data/save/schema migration IDs, forward/backward compatibility, irreversible operations, backup/checkpoint paths/hashes, restore procedure, and validation;
- ordered post-rollback health, data-integrity, smoke, and regression checks;
- rehearsal environment/deployment ID, start/end, exact steps, observed restored artifact, validation receipts/logs/hashes, result, and rehearsal receipt hash.

A prose command, shell string, branch/tag/dashboard label, unknown current artifact, missing rollback bytes/owner/triggers/validation, unavailable backup, incompatible data/save/schema, irreversible migration, NOT_REHEARSED, partial rehearsal, or stale hash blocks HOTFIX READY and RELEASE HANDOFF READY. This workflow never executes rollback.

## Phase 10: Assess one immutable local candidate

`assess` re-hashes plan/run/preflight/patch/review/commit/build/candidate, QA scope, task receipts, regression, smoke, current-production, rollback, rehearsal, and interface bundle.
...
8. every interface artifact and transitive hash verifies;
...
`record-candidate` creates `cgs-hotfix-bug-candidate-link/v1` only after HOTFIX READY. It binds Bug/Hotfix IDs; canonical bug path/hash/status; fix full commit/tree; candidate manifest ID/path/hash and candidate-identity hash; build ID plus build-receipt path/hash; artifact path/hash, platform/configuration and source identity; exact regression Test IDs/source hashes/execution-receipt/log paths/hashes; smoke scope/receipt; assessment and rollback plan/rehearsal paths/hashes; proposed deployment targets; creation owner/time; canonical link hash; and state `FIX_CANDIDATE`.
...
and `result: SUCCESS`. It binds and revalidates predecessor/checkpoint hashes;
request and authorization path/digest; authority issuer/scope/expiry and signature
...
versions; start/end/observed times; external IDs; expected/actual state/digest;
logs/raw-payload hashes; retry/reconciliation/rollback state; and its canonical
identity hash. The observed candidate/build/artifact/deployment must join the
...
hash-mismatched, `OUTCOME_UNKNOWN`, or `TIMED_OUT_UNKNOWN` receipt is not deployment
...
- stable monitor IDs, queries, thresholds, units, cadence, missing-sample policy, and raw receipt hashes;
...
## Phase 14: CAS persistence and final response

For each owned artifact:
1. preview exact path, operation, preimage/ABSENT, byte count, rendered hash, authorization layer, and non-writes;
2. re-hash every authority and input immediately before mutation;
3. stage on the same filesystem;
4. validate schema, stable findings, references, and transitive hashes;
5. compare-and-swap unchanged inputs and expected destination;
6. atomically publish the exact action set;
7. read back and hash every member.
...
| HF-006 | Phase 4 — stable finding IDs/fingerprints and exactly one correction plus one re-review; unresolved blockers end BLOCKED | Case 2; HF-STA-008, HF-STA-009, HF-PRO-005 |
...
| HF-012 | Phase 8 — smoke/team-QA/bug interfaces are exact versioned path/hash/identity artifacts with deterministic mappings; conversation and agent labels are invalid | Case 8; HF-STA-019, HF-STA-020, HF-STA-026, HF-STA-027, HF-PRO-011, HF-PRO-014, HF-PRO-018 |
