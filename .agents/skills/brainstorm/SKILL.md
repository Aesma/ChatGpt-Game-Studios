...
Invoke this workflow as `$brainstorm`.

Before the first file change, present the complete proposed changeset, listing every file and intended modification, and obtain one explicit approval. After approval, make all changes within that boundary continuously without asking again file by file. If the scope expands materially, stop, present the revised changeset, and obtain one new approval.

Arguments: `[genre or theme hint, or 'open'] [--review full|lean|solo]`. Treat bracketed values as optional unless the workflow says otherwise.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Ask the user directly** at key decision points throughout brainstorming:
   ...
   a direct question to the user to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in a direct question to the user with preset options.)*
...
Bundle these into a single grouped set of questions presented directly to the user with these exact question group labels:
- Question group "Experience" — "What kind of experience do you most want players to have?" (Challenge & Mastery / Story & Discovery / Expression & Creativity / Relaxation & Flow)
- Question group "Timeline" — "What's your realistic development timeline?" (Weeks / Months / 1-2 years / Multi-year)
- Question group "Dev level" — "Where are you in your dev journey?" (First game / Shipped before / Professional background)

Use exactly these question group names — do not rename or duplicate them.
...
**CRITICAL**: Present one direct question with a plain option list:
...
Do not encode product-specific tool parameters or depend on a particular form UI.
...
Ask these as a direct question to the users — derive the options from the chosen concept, don't hardcode them:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` through Codex subagent delegation in parallel before moving to Phase 5. Issue both subagent delegations simultaneously — do not wait for one before starting the other.**
...
Collect both verdicts, then present them together using a two-part a direct question to the user:
- Question group **"Pillars"**: present creative-director feedback. Options mirror the standard CD-PILLARS handling — `Lock in as-is` / `Revise [specific pillar]` / `Discuss further`.
- Question group **"Visual anchor"**: present the art-director's 2-3 named visual direction options. Options: each named direction (one per option) + `Combine elements across directions` + `Describe my own direction`.
...
**After identifying biggest technical risks, spawn `technical-director` through Codex subagent delegation using gate TD-FEASIBILITY (`.codex/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` through Codex subagent delegation using gate PR-SCOPE (`.codex/docs/director-gates.md`).**
...
5. Use the single changeset approval policy:
- Prompt: "Game concept is ready. Should the proposed changeset include `design/gdd/game-concept.md`?"
- Options: `[A] Yes — include it` / `[B] Not yet — revise a section first`
...
Once the complete changeset is authorized, generate the document using the template at `.codex/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
