...
5. Validate optional scope/prior-record manifest bytes, schema, project/target,
   normalized hash, and cursor before resolving stories.
...
Read only the exact canonical path and establish its raw hash/size. Missing,
ambiguous, unsafe, or over-limit identity is a one-story blocked/input result; do
not search for a replacement.

### Sprint

1. Read exact `production/sprint-status.yaml` bytes, compute its raw SHA-256, and
...
2. Validate positive tracker revision, stable event ID, equal sprint/active IDs,
   ...
   raw `plan_sha256`, plan revision, story-set hash, dates/timezone/unit, capacity
   receipt/operands, and complete story rows.
3. Resolve only the declared `plan_file`, hash exact bytes, require
   `cgs.sprint-plan/v2`, and validate plan ID/revision/raw hash plus exact tracker/
   plan payload agreement.
4. Recompute `story_set_hash` from every plan row and current story bytes; validate
...
6. Preserve plan order for display; use stable story ID order for canonical hashes.

Any unsupported/legacy schema, `plan_path`/`plan_hash` substitution, zero/multiple
plan, duplicate/missing ID or ACTIVE declaration, inactive state, missing story,
stale tracker/plan/story hash/revision/timestamp, incomplete row, capacity/payload
...
cursor inputs but emit a cursor only after completed-result hashes are known.
...
6. Hash raw bytes and validate internal IDs/statuses/relationships.
...
8. Build `cgs.story-readiness-context/v1` with adapter/producer bundle,
   Source Manifest, Story Core, and currentness identities, then hash it.
...
1. story ID/schema, exact v2 adapter/producer contract, identity fields, Source
   Manifest ID, Story Core hash, complete currentness matrix, and production
   classification;
2. systems-index → Approved GDD path/hash → requirement ID;
3. story TR → current registry → same system/GDD/requirement;
4. governing ADR paths/hashes/status;
...
7. dependency IDs/paths/kinds/current hashes/status/cycles;
...
5. Normalize ADEQUATE/GAPS/INADEQUATE or mark only the affected story
   `UNVERIFIED` for missing/invalid/timeout/hash mismatch.
...
presence, sizes, and raw hashes. Also recheck sprint tracker/plan identity for a
...
3. match current/prior findings by `srf-v1` fingerprint;
...
1. verify ordered check rows and finding set/hash;
...
3. bind adapter/producer SKILL/contract/bundle, Source Manifest ID, and Story Core
   hash, then
   compute readiness key, stale key, checked-at, full candidate hash, and ID;
