...
| `evidence-backed-hard-constraint` | Current owned requirement/engine/platform/security/legal/compatibility evidence directly constrains alternatives; preserve source identity/revision and do not ask the user to vote it away. |
...
Every constraint has stable ID and:
    ...
    source_refs: [{id, path_or_url, locator, revision, version, observed_at}]
...
- `ADRC-META-01`: canonical UUID/allocation receipt/path/display sequence/slug are
  mutually consistent and target uniqueness atomic conflict check is current;
...
- `ADRC-ENG-01`: pinned engine/version/domain/risk/cutoff and every consulted
  reference ID/owner/path/locator/revision/date/coverage state are explicit;
...
- `ADRC-DEP-02`: graph records unique resolution, current lifecycle status/revision,
...
- `ADRC-CON-01`: all constraints use ADRCON IDs, owners, applicability, source
  revisions, verification, and coverage state;
- `ADRC-CON-02`: every current product/technical requirement has stable ID/owner/
  path/locator/revision and separates preserved product rule from ADR realization.
...
- `ADRC-PERF-01`: every numeric baseline/budget/prediction has source ID/path/
  locator/revision/units/method/date; otherwise it is labeled HYPOTHESIS/UNKNOWN;
...
- `ADRC-REPL-01`: related/depends/supersedes/scoped-exception links use exact IDs/
  paths/revisions/current statuses and no replacement cycle;
...
- `ADRC-PROV-02`: exact-body approvals, revisions, consultations, findings, graph,
  context and authorization revisions are current and noncontradictory.
...
Use `cgs.adr-dependency-finding/v1`:
    ...
    id: ADRF-<adr-uuid>-<check-id>-<stable key>
    ...
    evidence: {path_or_url, source_id, locator, revision, observed}
...
stable key deterministically covers category, owner, source ID/locator,
...
`dependency_may_be_unblocked` event with ADR path/revision, affected IDs, and
...
## Phase 8: Section drafting, semantic preflight, approval, and atomic conflict check
...
3. revalidate duplicate stable key, dependency/replacement graph, owner
...
6. re-read target/section/context/authorization/draft bytes; and
...
Store actual content approval:
    ...
    graph_and_finding_revision: <revision>
    context_manifest_revision: <revision>
    target_and_section_baseline_revision: <revisions>
...
1. **Target atomic conflict check** — current raw target equals expected baseline/last verified revision;
2. **Section atomic conflict check** — selected stable-ID body/unique anchors equal approved
   prior state and all out-of-scope ranges match;
3. **Context atomic conflict check** — every external source used plus ordered manifest reference ID match;
   mutable target is excluded here;
4. **Authorization atomic conflict check** — exact path/operation/section/limits, approval revision, and
   authorization identity/revision match; and
5. **Writer atomic conflict check** — actual target writer equals authorized identity.
...
Then append checkpoint by predecessor/create-if-absent atomic conflict check. If checkpoint fails
after verified target bytes, preserve content, return Workflow Verdict PARTIAL,
report exact unreceipted target revision, emit no review handoff, and never replay.

## Phase 9: Whole-ADR validation and authoring receipt

Re-evaluate exact single-decision scope, stable key/duplicates, every section/
...
target/context revisions, and preserved bytes.
...
Before final metadata write rerun five-part atomic conflict check. Then create final external
`cgs.adr-authoring-receipt/v1` only after stable final target bytes. It binds
allocation/request/operation, pre/post target/section revisions, profile/content/
author schema, context reference ID, scope/stable key, sources/constraints/alternatives/
...
Receipt ID may appear in target; receipt path/revision may not. Receipt is authoring
...
- CONTENT_COMPLETE target but receipt append/verification fails: PARTIAL, no
  handoff, exact unreceipted target revision, no replay/revert;
...
- exact ADR ID/path/current revision and profile/content/author schema;
- single-decision scope/stable key, all section/assertion revisions;
...
- context reference ID, authoring receipt ID/path/revision, and all author/consultant/writer/
...
verified authoring receipt, Proposed target prior state, dependency/replacement
eligibility, and identity separation. It performs exact atomic conflict check transition and emits
`cgs.adr-lifecycle-record/v1` binding review record/revision, status-only pre/post
target revisions, recorder identity, timestamp, and any atomic predecessor/successor
...
chain, allocation/target/context/schema, authorization/roles, scope/stable key,
...
Conversation memory never reconstructs source revisions, decisions, approval,
checkpoint, receipt, review, lifecycle state, or acceptance.

Final response lists run/ADR/allocation IDs, exact path/operation, scope/stable key,
target pre/post/revision, section axes/assertions, sources/coverage, constraints/
alternatives/decision/approval/revisions, graph/replacement links, consultations/
findings/handoffs, authorization/roles, checkpoint/receipt paths/revisions,
