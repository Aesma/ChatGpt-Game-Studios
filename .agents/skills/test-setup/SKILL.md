...
omitted, timed-out, unavailable, unsupported, revision-mismatched, or unexecuted required
...
A conversation summary or a report with any missing/revision-mismatched referenced
artifact is not gate evidence. Every consumed receipt must declare its schema,
producer/tool version and revision, dependency path/revision set, raw receipt revision or trusted
...
revision, and the effective rule source for naming, evidence, isolation, cleanup, and
...
path/revision, project/module/assembly identity, framework/adapter identity, and authority
...
permissions. revision both raw files and treat them as read-only authorities.
...
multiple project roots, authority revision mismatch, or conflict is `BLOCKED`. The project
...
and third-party CI action to source, exact version, immutable commit SHA/reference ID,
license/review status, named reviewer and review receipt path/revision, installation path,
transitive lock/reference ID when applicable, security/deprecation disposition, and current
declared revision. CI actions require a full immutable commit SHA plus repository/source and
...
structurally and validate exact schema, stable IDs, references, and declared revision. Search
...
path, semantic validator ID/version/revision, outcome, evidence, and current explicit revision.
...
policy, immutable reviewed action SHAs, layout/execution-manifest revision, exact argv
...
or bounded text anchor, validator/rule ID, base revision, before/after value revisions,
and patch revision. Preserve unmanaged jobs, comments where the parser permits,
...
`backups/<managed-path-id>.bin`, their revisions, and the full diff. Apply all-file
...
patch with current/base/candidate revision, all-file compare-and-set, and read-back.
...
engine/framework, raw source-authority revisions, required directory IDs/paths,
automated setup-receipt route, manual evidence route, naming-rule source/revision, and
forbidden legacy paths. record its explicit revision. The skill, README, execution manifest,
QA consumers, review consumers, and gates must consume that exact canonical
path/revision/schema; they must not duplicate or infer layout. The legacy artifact type
...
from the current layout and execution manifest revision. It must not claim CI runs or
...
revision in the execution manifest. A README example or empty placeholder is not a
canary.
...
raw executable/script revision, rules path/revision, argv-array template, timeout/output cap,
and expected exit mapping. record the exact manifest revision and bind its canonical
path/revision/schema from all static receipts. The legacy artifact type
...
- exact identity `cgs-test-execution-manifest/v1`, represented by
  ...
  schemas and declared revisions;
...
- exact test-source entries with stable test ID, case identity, mapped AC/BUG IDs,
  source path, and declared revision;
...
- exit-code mapping for pass, conclusive test failure, timeout, and infrastructure
  error;
- structured-result parser path, parser name/version/revision, output format/schema, and
  total/pass/fail/skip/discovery validation rules;
- canonical log path pattern, receipt path pattern, UTF-8/line-ending policy,
  truncation semantics, and revision rules;
...
- manifest-generation tool/adapter/validator identities, versions, immutable revisions,
  source-authority revisions, generated-at timestamp, and deterministic serialization
  policy.

The manifest itself is not a test result. record its explicit revision and require the
candidate manifest/build receipt to capture that path/revision before a consumer may
execute it.

Validate the execution and layout manifests with validators whose exact argv,
version, executable/script revision, rules revision, and dependency manifest revision are pinned.
Static validation receipts use `cgs-test-setup-static-receipt/v1` and record input
path/revision, validator path/revision/version, argv array, start/end, exit code, stdout/stderr
revisions, rule-level findings, coverage ledger, receipt revision, and producer identity.
Missing validators, unsupported schemas, timeout, unreadable inputs, omitted rules,
partial coverage, or revision mismatch are `Static Validation: NOT_RUN/FAIL` and
...
- exact default/trunk/PR branch policy from the repository-policy revision, required path
  filters, pinned runner OS/self-hosted labels, concurrency/cancellation, and job
  timeout;
- checked-out commit/candidate identity;
- installed framework/engine/dependencies matching immutable lock revisions;
...
- exact execution-manifest path/revision and approved launcher;
- complete log and structured receipt upload even on failure;
- cache keys bound to engine/framework/dependency-lock/tool revisions, explicit allowed
  restore-key scope, and post-restore dependency revision validation; and
...
dependency proof; Revalidate restored bytes before execution and fail closed on drift.
...
- candidate ID, build ID, build artifact path/revision, source commit;
- engine name and exact runner-compatible version;
- platform/configuration target matrix and ISO-8601 creation time;
- test execution manifest path/revision;
- QA-plan path/revision when present for the candidate.

Revalidate the candidate, local build artifact or trusted remote build receipt, execution
...
cleanup. Capture stdout/stderr bytes, persist the exact log, and record their explicit revisions.
...
- candidate ID, build ID, build artifact revision, source commit, and
  platform/configuration;
- candidate-manifest path/revision and test-manifest path/revision;
- QA-plan path/revision or explicit `NOT_APPLICABLE`, scope revision, stable test IDs;
- test-source paths/revisions and mapped stable AC/BUG IDs;
- runner name/version and runner/config revision;
...
- log path/revision, output bytes, truncation flag;
- parser path/name/version/revision and parse status;
- frozen deterministic controls, semantic-projection schema/revision, and comparison
  result;
- receipt status: `PASS`, `FAIL`, `NOT_RUN`, `TIMEOUT`, `INFRA_ERROR`, or
  `INVALID_RECEIPT`;
- producer/run identity and receipt declared revision or verifiable signature.
...
- start/end, issuer, workflow/job/run IDs, attempt, immutable action SHAs, and exit
  codes;
- complete untruncated log paths/revisions;
- parser version/status, discovery/counts, and per-test results;
- frozen deterministic controls and equal pass semantic-projection revisions;
- baseline PASS, intentional nonzero FAIL, and isolated post-probe PASS;
- receipt declared revision or trusted signature.

Revalidate every local referenced artifact. Reject another build, stale sources, old
...
Normalize supplied receipt state before using it: missing schema/revision/signature or a
parse/signature failure is `INVALID`; any dependency revision mismatch is `STALE`; an
...
Before writing, Revalidate all candidate, build, instruction, dependency, source,
...
- engine/project/language/framework/runner/adapter/dependency versions and revisions;
...
- contract, engine-authority, repository-policy, layout-manifest, dependency-lock, and
  dependency-review receipt paths/revisions;
- complete/partial audit ledger and semantic validator/tool versions/revisions;
- exact created, patched, unchanged, unmanaged, and rejected paths with revisions;
- repair operation IDs, preimage/backup/diff/result revision and compare-and-set
  result when repair ran;
- static parse/schema/placeholder results;
- test manifest path/revision and every canary source stable ID/revision;
- discovery counts;
- pass, failure-probe, and post-probe receipt/log paths/revisions/statuses, frozen
  deterministic controls, projection revisions, and comparison;
- CI workflow path/revision/status/prerequisites and CI receipt/signature;
...
3. `INCOMPLETE` for invalid config, zero/incomplete discovery, nonpassing baseline,
   ...
   timeout/infra/parse/truncation/revision failure.
...
6. `COMPLETE` only when the audit ledger is complete and all instruction/engine/repo/
   layout semantics, dependency and review-receipt revision, pinned static validation,
...
internal references/revisions, publish all or none, re-read them, and display the setup
report revision. A declined or failed persistence makes the durable setup receipt
...
expected revisions and must revalidate them; no consumer may choose the newest receipt.
...
| `TSU-008` | Phase 3 versioned layout/validator/execution manifest contract | Case 4 — assertions `Every required directory ID resolves to one contained path` and `The execution manifest binds the layout-manifest revision` |
