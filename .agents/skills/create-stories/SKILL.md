...
outside-root or escaping symlink path, invalid batch/cursor, malformed hash, an
evidence path without its expected hash, or both path/inline forms.
...
Record exact states/hashes for catalog, EPIC, create-epics receipt, STORIES
...
requirement/criterion require the exact current approved GDD identity/path/hash,
stable source ID or exact approved fingerprint, locator/excerpt hash, and current
...
source requirement/approval hashes, `CURRENT`, and
...
For every TR/slice, validate exact ADR ID/path/hash, Accepted lifecycle record,
...
Index story ID, canonical key hash, never-reused slot, path, revision/history,
...
and `STORY-<16hex>`. Reuse an existing identity only when exact stable
...
paths/core hashes. Record HARD/SOFT plus reason/source. Reject fuzzy/title
...
planning evidence, with source ID/locator/hash. Missing/ambiguous priority is
...
Every AC binds one exact approved source criterion identity/locator/excerpt hash
...
Use a QA plan only from the explicit path/hash or unique exact epic link. Validate
its current state and exact tuple:

```text
(qa_scope_or_sprint_id, epic_id, story_id, canonical path,
 captured_story_artifact_sha256, story_core_sha256, ac_set_sha256,
 AC ID, Test ID)
```
...
Compute canonical `story_core_sha256` and AC-set hash before QA content so exact
...
and every story ID/path/core hash, type, AC/Test mapping/body, and limitation.

Require a distinct per-story `ADEQUATE|GAPS|INADEQUATE|UNKNOWN` record, stable
finding IDs, exact received core hash, coverage by AC/Test ID, real reviewer
...
priority/provenance, source manifest/core hash, exact source matrix, rule IDs,
...
a later exact-hash story-readiness result owns final READY.
...
and writes nothing. The artifact never embeds its own current file hash.
...
epic/source/inventory/order hashes and remaining stable IDs. Never persist cursor
or session state.
...
- every selected story CREATE/UPDATE/RETIRE path, preimage, candidate bytes/hash,
  and stable-ID diff;
...
Immediately before mutation perform the reference contract's full CAS: rehash the
...
create/replace. Verify exact bytes/hashes/schema/identity/revision/status/readiness/
...
PARTIAL_WRITE with exact verified/uncertain/unattempted hashes. Never delete a
...
batch hashes, story operation/status/readiness, exact AC/Test/dependency IDs,
...
4. invoke story-readiness for the first exact AUTHOR_COMPLETE path/hash; or
5. Stop.
