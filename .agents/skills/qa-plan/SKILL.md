...
- **`sprint`** — resolve the currently active sprint from an existing active/status
  reference and verify the referenced sprint plan before extracting story paths.
...
| [story title] | Integration | Integration test OR documented playtest | Smoke check |
