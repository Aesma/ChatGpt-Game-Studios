...
description: "Bounded, revision-bound, read-only architecture traceability gate over explicit approved requirements, ADR decisions, engine constraints, and current execution evidence; returns PASS, BLOCKED, or PARTIAL."
---
...
This skill is a formal, revision-bound gate. Its run verdict is exactly `PASS`,
...
- **Explicit baseline only:** admit only stable requirement IDs backed by exact
  source text, current source version/revision, lifecycle state, owner identity,
...
- **Machine identity:** every report binds a complete path/revision manifest,
  ruleset revision, skill-bundle revision, unique run ID, target manifest revision, stale
...
5. Build the mutation baseline as a streaming path/size/revision tree outside
...
- `full`: all classes required by coverage, consistency, and engine, plus the
  ...
  index may supply stable identity only when used and must then be versioned. A
...
Validate each intended input’s declared identity, schema, version, and revision, but first load only bounded declared
sections/fields needed to build indexes:

- requirement ID, source path/revision, lifecycle, owner, approval, criticality,
...
their revisions before and after review.
...
- canonical project-relative path and complete-file revision;
...
exclusion, input-class ledger, ruleset ID/revision, effective limits, planned shards,
and skill-bundle revision over the ordered main file, continuation, and ruleset
bytes.

`target_manifest_revision` is the explicit revision declared by the manifest authority. `stale_key` is the ordered tuple of project ID, mode, target ID, ruleset ID/version, and source IDs/revisions.
`run_id` is
`AR-<UTC timestamp with fractional seconds>-<UUIDv4>`. Generate the
...
3. Record names the source GDD path and exact source version/revision in the
   manifest;
...
current usable ADR explicitly names the exact requirement ID, the ADR revision is in
...
- `EXECUTED_PASS` — authoritative latest run for the exact test revision and current
  source revision/manifest passed;
- `EXECUTED_FAIL` — authoritative matching current run failed;
- `STALE_RUN` — run targets another test revision, revision, manifest, or link set;
...
An authoritative run contains immutable run ID, exact test path/revision, linked
IDs, result, execution timestamp, and reviewed revision or manifest revision. Only
...
append-only sequence/index with a validated record revision. Never choose a test run
...
pinned current VERSION and directly applicable reference revisions. Missing,
...
target manifest revision, ruleset excerpt, check IDs, and exact input revisions. It is
...
target_manifest_revision: <revision reviewed>
input_manifest: []
checks: []
unchecked_scope: []
findings: []
...
`DECLINED`, `TIMEOUT`, `ERROR`, wrong manifest/revision, missing required check, or
...
identical finding keys while retaining provenance. Incompatible facts,
severity, or evidence for the same finding key are `EVIDENCE_CONFLICT`, not a
...
when exactly one indexed path, record revision, project ID, mode, target identity,
...
Rebuild the prior report's recorded scope and stale key. Any path, revision,
revision, required class, target selector, mode, ruleset revision, or scope change
...
revision, report/finding IDs, exact scope/manifest, owner/signature, timestamp, and
