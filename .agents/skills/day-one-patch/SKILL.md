...
`$day-one-patch --request <path> --expect-request <sha256>`
...
Every mode declares project root/identity, exact root-to-output `AGENTS.md` path/hash
chain, release/patch policy and owner-registry paths/hashes, hard budgets, output root,
trusted time/cutoff receipt and non-writes. Additionally:

- PLAN names the exact gold-master build/release manifests and artifact hashes,
...
- RESUME names one exact immutable checkpoint path/hash and expected predecessor chain.
...
6. publication/notification — separate human authority for exact channel/account/
   message digest/time/idempotency action.
...
Limit/timeout/unreadable/omitted evidence returns INCOMPLETE with exact counters, hashes,
reason and identity-bound resume cursor; no partial set can become ready.

## Canonical identities and states

Strictly parse and hash exact raw bytes before canonicalization. Canonical serialization
is UTF-8/LF/NFC, schema field order, sorted sets and stable ID order.

```text
base_candidate_identity_sha256 = sha256(release + candidate/build/artifact/source +
  platform/configuration + build/release manifest hashes)
patch_item_identity_sha256 = sha256(patch/source issue + owner/allowed/prohibited paths +
  budgets + acceptance/tests/rollback/risk + source evidence hashes)
plan_identity_sha256 = sha256(request/base/policy/owner/instruction identities + ordered
  included/deferred item hashes + rollback/smoke/evidence-template hashes)
new_candidate_identity_sha256 = sha256(new candidate/build/artifact/source/platform +
  build manifest/receipt + exact diff manifest)
evidence_snapshot_sha256 = sha256(plan/new candidate/release checklist + ordered
  implementation/bug/repro/regression/smoke/risk/rollback evidence hashes and states)
gate_identity_sha256 = sha256(plan/new candidate/policy/instruction/evidence identities +
  ordered finding/row hashes + S1 disposition + gate verdict)
```

Stable patch item ID is `DOPI-{source-id}-{first12(patch_item_identity_sha256)}`; it is
not an ordinal dependent on discovery order. Stable finding ID is
`DOPF-{patch-id}-{item-or-document-id}-{rule-id}`. Do not recycle IDs across changed
scope; retain supersedes lineage.
...
Read/hashes request, instructions, gold-master build/release manifests and artifact,
patch/release policies, owner registry, target matrix and exact bug/cert evidence index.
Read in full the declared root-to-output AGENTS chain, verify order/hashes and apply
...
transition history and raw hash. Do not infer lifecycle/severity from filenames,
...
- stable item/source bug-or-cert IDs and raw hashes, severity/status and requirement;
- immutable owner ID/role/registry hash and implementation-receipt producer;
...
compensating controls, monitoring/rollback/communication, issued/expiry and hashes.
...
3. `cgs.day-one-patch-evidence-manifest/v2` template listing every required producer,
   artifact type/schema/path/hash placeholder, candidate binding and deadline; and
4. `cgs.day-one-deployment-observation-plan/v1` proposal with no authority.

Every claim/finding/item links stable IDs and immutable source hashes. Free-text
commit/tag/build slots are not evidence. The bundle records `plan_identity_sha256`,
...
- save/data/schema compatibility in both directions, migration reversibility,
  backup/checkpoint/restore hashes and irreversible-operation handling;
...
missing/stale/partial/wrong-build/wrong-environment is INCOMPLETE. A report hash alone
...
receipt hashes. Timeout/cancel/error becomes TIMEOUT/NOT_RUN/UNAVAILABLE at gate, never
skip/pass. No late receipt may mutate a frozen/persisted gate report.

Each `cgs.day-one-smoke-request/v2` contains:

- request/plan/item IDs/hashes and expected response schema/version;
- mode `QUICK_TARGETED` or `SPRINT_FULL`;
- exact new build-candidate manifest/candidate/build/artifact/source/platform identity;
- exact QA-plan/test-manifest paths/hashes and ordered stable QA check IDs;
- canonical scope serialization/hash, exclusions, timeout and output report path; and
...
the plan/QA policy. Re-hash candidate/QA/test/scope/automation/manual/log dependencies.
...
ordered v2 items, implementation and diff receipts, actual changed paths/hashes, owner,
...
Re-hash every input/dependency. Actual diff must be a subset of each allowed path and
...
full candidate/checklist identity path, exact report hash, release-policy/instruction/
evidence identities, ordered row hashes, recorder CREATED, and `Gate Decision:
NOT_EVALUATED` with Release/Deployment/Publication Authority NONE.

Re-hash every referenced dependency and independently apply the release policy. The
...
finding IDs, paths/raw hashes, candidate/platform binding, state, owner, reason and row
hash. Preserve all failing/incomplete rows even when an earlier rule determines verdict.
...
```text
production/releases/{release-id}/day-one-patches/{patch-id}/plans/{plan-identity-sha256}.yaml
production/releases/{release-id}/day-one-patches/{patch-id}/gates/{gate-identity-sha256}.md
production/releases/{release-id}/day-one-patches/{patch-id}/checkpoints/{checkpoint-identity-sha256}.yaml
```

All digests are full lowercase SHA-256. analyze-only writes nothing. For recording,
preview exact bytes/hash/target expected ABSENT, all input identities, recorder,
create-new primitive, size, authority/expiry and non-writes. Immediately before create,
CAS every input/dependency/owner/candidate/artifact/evidence/output hash, parent identity
...
decisions, outputs/hashes and next legal idempotent step. RESUME re-hashes the entire
...
external job/submission ID, observed installed digest/result, rollout/hold stages,
...
smoke/release/rollback rows; stale/partial/not-run/timeout/unknown owners; target/hash or
