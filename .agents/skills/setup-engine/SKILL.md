---
name: setup-engine
description: Research, select, verify, configure, refresh, or upgrade a game engine using revision-pinned official-source claims, real executable and toolchain health receipts, owner-safe transactions, and explicit partial/offline recovery states.
---
...
```text
$setup-engine --manifest <engine-request-path> --manifest-revision <revision>
              [--resume <checkpoint-path> --resume-revision <revision>]
```

Manifest path/revision are required together. Resume path/revision are optional but inseparable. Validate arguments before repository reads beyond the manifest, network access, execution, delegation, decisions, or writes. With no manifest, show usage and stop with zero side effects and no verdict. Reject unknown/repeated flags, moving aliases such as `latest`, directories, unsafe IDs, traversal, symlink/junction/reparse escapes, unsupported schema, and expected/actual revision mismatch.
...
- expected executable path or bounded installation roots, expected binary revision/signature when pinned, exact version argv/parser, required SDK/compiler/runtime/package/export-template commands and locks, and project health/build/test commands;
- authoritative testing-configuration/catalog path and expected revision or explicit `UNCONFIGURED`;
- exact project/config/reference/evidence/checkpoint/receipt/transaction paths, owners, expected base revisions or ABSENT, writers, limits, and non-writes;
...
- `UPGRADE_VERIFIED` — target installation, isolated migration/import/build/regression, activation, and post-activation health all passed on one final revision;
...
Qualify every statement such as version available, supported, downloadable, licensed, configured, healthy, or upgraded with exact product/edition/version/channel/platform/region/as-of time and evidence/receipt revision. Never emit an unqualified `latest`, `free`, `supported`, or `verified` claim.
...
- HTTP/result status, final URL and redirect chain, ETag/Last-Modified, response/content revision, and preserved normalized finding;
- for download claims, exact artifact name/bytes/platform/architecture, official download URL, checksum algorithm/value and separately sourced official checksum URL/revision, or signature/certificate/notarization identity and validation policy;
...
“Most recent” can only mean the maximal release under the exact official archive snapshot and channel/platform predicate at `retrieved_at`; retain the archive URL/explicit revision and do not call it timeless/latest. Fixed training/knowledge-cutoff prose is forbidden.
...
- exact engine product/edition/version/channel and license/terms document title/version/URL/revision;
...
- user-provided applicability facts/attestation revision, with sensitive exact revenue minimized to an adequate band when possible;
...
Every record uses canonical serialization, schema version, request/run/installation IDs, producer/tool version, source/input/output revision, authority IDs, UTC time, and status. Read back every persisted record and retain its revision. An existing filename, old receipt, timestamp, or status word is not current evidence.

revision this `SKILL.md` and `references/continued-workflow.md` in the initial packet. Revalidate both before the continuation; mixed contract versions are BLOCKED.
...
4. Verify publisher ownership, redirect eligibility, source freshness, response revision, and claim applicability.
...
Source conflict is not resolved by recency alone. Preserve both claims/scopes/revisions and mark dependent facts UNRESOLVED. Missing mandatory release/support/download/checksum/license/platform evidence blocks dependent decision/install/configuration.
...
Present only evidenced candidates. Each option shows exact product/edition/version/channel, release/support horizon as of a date, download/checksum/signature status, platform/toolchain constraints, license region/date/threshold context, unresolved facts, and source IDs/revisions.

The named user/product authority chooses. The workflow may compare tradeoffs but cannot silently decide. Record decision ID, exact candidate, research/license manifest revision, alternatives, verbatim choice/rationale, decision identity/source, timestamp, and accepted unresolved risks.
...
Read only the exact manifest-declared authoritative testing catalog/configuration and expected revision. Record `UNCONFIGURED` if none exists; do not invent a default.

Configuration/upgrade candidates reference the catalog entry by stable framework/config ID, version, path, and revision. The project-health/test command must come from that exact configuration, not an inline engine-specific recommendation.

Emit `cgs.engine-framework-consistency-receipt/v1` with engine/language/project config revisions, testing catalog/config path/revision/schema, framework ID/version, adapter/runner command receipt, expected/actual discovery result, and status:
