...
2. Require exact `path` pair for compare/inspect and one exact
...
only canonical active-state pointer fields, revisions the exact suggested candidates,
...
Record beforevalidate for baseline/current. Enforce the per-artifact byte limit,
...
- current baseline reference matches supplied path/revision/ID/version;
- compatible normalization schema/adapter; and
- declared counts, unique stable Scope IDs, and explicit removals.

Follow only exact mandatory approval/authority/normalization `path@revision`
...
Any actual/invocation revision mismatch, changed baseline at same path, missing or
...
## Phase 3: Normalize and revision entries

Load only semantic fields declared by the shared normalization adapter. Canonicalize
and revision each Scope ID payload and acceptance boundary. Preserve source locations
and exact artifact revision.

Ignore presentation order/heading/row formatting. A split/merge is presentation
only when the same stable IDs normalize to the same semantic revisions. Never use
...
Silent deletion is conflict. Record stable finding key/Delta ID without artifact
revisions or presentation fields. Store both artifact/entry revisions and semantic
...
3. match exact baseline/current/entry revisions, parent, and timebox;
4. validate decision owner through exact authority record;
5. validate timestamp/signature/record identifier/supersession;
...
limits. Read only its exact path@revision rows and exact Git IDs/range. Do not follow
...
and pair revisions. Git/code/TODO/build evidence may show implementation activity but
cannot create scope or approval.

Keep every unavailable/revision mismatched/unsupported/over-budget row as unchecked.
...
Validate estimates for all changed IDs against exact pair/entry revisions, calibrated
...
dimension/exposure, pair/evidence revisions, owner authority, rationale, controls,
...
- Unreadable revalidate -> same severity as the affected role.
...
- stable sorted deltas and both-side revisions;
- authority, allowed, risk, effort, and impact states;
- core/evidence coverage and every unchecked identity;
- input before/after revisions, comparison/stale keys, result, and neutral options;
- `NOT_PERSISTED`, gate-ineligible, and `READ_ONLY` boundary.

End with exact baseline/current path@revision identities and state that no file, Git
