...
2. All sections are authored, previewed as one bounded document changeset, and then written under one authorization
...
4. Skill authors only that section, asks "May I apply the proposed changeset?"
...
- The retrofit mode for updating multiple sections in one session follows the
  same bounded document-authorization pattern — not independently tested for multi-section
  retrofits.
