...
- [ ] SD-S016: Manual evidence binds identity, session, steps, observation, tree/build, artifacts, and revisions
...
- [ ] SD-S021: Review policy is predeclared and revision-bound
...
- [ ] SD-S024: Functional equivalence requires an authorized owner decision with exact source/subject revisions
...
- [ ] SD-S027: Story-done never writes tracker, story, plan, session, planning revisions, or evidence artifacts directly
- [ ] SD-S028: Closure proposal binds exact tracker atomic conflict check tuple, one row transition, lifecycle-owned fields, preservation assertions, rollback, and authorization
...
- Immutable `cgs.story/v2` ID/path/revision matches one tracker row exactly `in_review`
- Current persisted READY record, matching recorder receipt, and every stale-key source revision match
...
- [ ] SD-C01-A: Exact dev plan/execution command contexts and log revisions support each AC
...
plan, and dev-story result revisions.
...
- [ ] SD-C05-A: Identity/session/behavior/artifact/revision fields are all enforced
...
### Case 6: Readiness is checked on current revisions
...
pair whose registry-final revision no longer matches current bytes.
...
No ad hoc command, replacement result/plan/receipt, story projection, lifecycle normalization, or planning-revision rewrite is invented.
...
log-revision mismatch, and evidence-tree mismatch variants for one declared test.
...
tree, and matching log revisions can support the mapped AC.
...
denominator, scope/candidate/build/artifact/source/plan revisions, roles, findings,
conditions, persistence, and raw revisions.
...
revision. Exercise lean and solo modes plus a user answer of No/skip.
...
waiver bound to policy/story/plan/result/tree revisions. Variant B has a policy
...
- [ ] SD-C11-B: Waiver scope and revisions match the exact current candidate
...
- [ ] SD-C13-A: Decision scope and current revisions are exact
...
### Case 14: Lifecycle recorder unavailability or atomic conflict check conflict prevents canonical writes
...
Tracker, receipt path, story, plan, planning revisions, session state, implementation, and sources.

#### Expected behavior

Abort before commit and return `BLOCKED` with the exact conflict; do not retry against a new prior state.

#### Assertions

- [ ] SD-C14-A: Story-done direct tracker replacement and sequential cross-owner writes are forbidden
- [ ] SD-C14-B: Full tracker/story/plan atomic conflict check and preservation checks precede recorder commit
...
### Case 15: Injected lifecycle-recorder failure restores the tracker prior state
...
Proposal, staged bytes, tracker prior state, immutable story/plan revisions, recorder journal, and rollback data.

#### Expected writes

Only recorder-controlled rollback/journal/immutable aborted-receipt writes permitted by its capability; final tracker bytes equal the complete prior state.

#### Expected non-writes

No story, plan, session, planning-revision, evidence, or independent repair write.
...
No normalization of terminal state, story rewrite, tracker rewrite, planning-revision rewrite, or new closure receipt.
...
Exercise `ADEQUATE + UNKNOWN`, `ADEQUATE + PASS + TARGETED`, stale revisions,
...
revision plus the direct AC evidence.
...
A fully authorized tracker-only closure proposal with current atomic conflict check prior state.

#### Expected reads

Immutable story, plan, tracker prior state, dev transition receipt, closure proposal/authorization, recorder capability, and receipt destination.
...
- [ ] SD-C18-A: Story-set revision is preserved byte-for-byte and never recomputed
...
- [ ] SD-P002: Use declared revision for every mutable and evidence boundary
...
- [ ] SD-P007: Re-preview after any path, byte, prior state, authorization, or intent change
...
| SD-004 | SD-S016 and Case 5 require revision/session/identity-bound manual evidence |
...
| SD-010 | SD-S026–SD-S031 and Cases 14–15/18 require tracker-only atomic conflict check closure, immutable planning/story inputs, rollback, and receipt |
