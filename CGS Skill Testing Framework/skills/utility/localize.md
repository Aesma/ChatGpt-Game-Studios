...
Validate `localize` as an owner-separated, version-bound source-catalog, locale-package, freeze, review, and runtime-evidence workflow. Templates, populated values, machine drafts, static checks, pseudolocalization, and QA plans never become completed localization or release proof.
...
```text
$localize <subcommand> --request <path>
```
...
2. ACTIVE freeze binds manifest/catalog/source/keyset/per-key revisions and rejects catalog mutation before write unless a complete owner change request lifts it.
...
6. Every write uses exact path/owner/writer/base/candidate authorization, version and existence conflict check, read-back, and rollback.

## P1 requirements

### A. Strict canonical formats and identity revisions — LOC-005

1. `cgs.localization-manifest/v2` declares project/source locale, catalog schema, exact table path/format/schema/parser/version, key policy, placeholder syntax/parser/version, plural rule source/version/revision, and each target locale path/format/schema/owners/fallback chain.
...
3. Manifest/source table declared revisions, canonical keyset revision, per-key source revision, catalog identity, locale identity, and translation revision identity are distinct and reproducibly derived.
...
3. Rename requires old/new IDs, `supersedes_key`, owner change request, affected locale revisions and migration/retranslation scope.
4. Source text revision changes per-key revision; retaining key requires source-owner meaning-continuity confirmation.
...
7. Required plural categories come only from versioned locale rules/revision. Missing/extra-invalid/renamed/type-changed placeholders or malformed plural/select syntax block import and cannot auto-fix.
...
3. Pseudolocalization records algorithm/version/seed/settings plus exact catalog/source/keyset and generated revisions and is always TEST_ONLY_NOT_TRANSLATION.
4. Runtime proof binds exact build ID/revision, platform/config, viewport/DPI/direction, locale/pseudo identity, catalog/source/keyset/translation/freeze/font/scene/test identities, tester/times/results and screenshot/video/log revisions.
...
3. `cgs.locale-review/v1` binds a distinct locale-qualified reviewer, exact revision/revision/key range, findings/decision/times and receipt revision.
...
1. `cgs.localization-change-request/v1` binds owner/authority/reason, affected key IDs and frozen revisions, intended delta, impacted locale identity+translation revision set, retranslation/re-review scope, vendor notification proposal/receipt, privacy change, approval time/revision and expiry.
...
4. Successful catalog apply revalidate identities and marks affected translations/reviews/QA evidence STALE.
...
2. Worker receives exact input revisions and schema, returns complete response with attempt token/status/revision.
...
2. It revalidate locale/build/platform, manifest/freeze/catalog/source/keyset/per-key/translation/font/VO/scene/test/evidence revisions, freshness and required coverage.
...
6. Only a conclusive complete review emits a non-persisted `cgs.review-evidence/v1`
   ...
   is the path/raw-version-bound `cgs.localization-evidence-manifest/v2`, its
   `artifact_revision` equals that declared revision, and its persistence/receipt remain NONE.
7. Durable gate evidence additionally requires an independent current
   ...
   review bytes, generic record ID, extension path/revision, request/manifest/catalog/
   ...
   preimages, version and existence conflict check creation and verified read-back.
...
2. PRESENT consumes only `cgs.narrative-localization-handoff/v2` through
   ...
   revision/handoff/content/run/authority IDs. No alias or heuristic selection exists.
...
## Parent-requested owner/version and existence conflict check and privacy contracts

### Import/export owner and version and existence conflict check

Export creates an immutable source package only; it never creates translation or contacts a vendor. Exact `cgs.localization-package/v1` fields bind the owner-assigned package ID, locale/page, manifest and catalog paths with explicit revisions, source-table/keyset/catalog/locale/plural/placeholder identities, ordered per-key source revisions, context/glossary/do-not-translate ID, privacy, owner, explicit payload revision, and separate declared file revision. No identity is calculated from package content. Exact package paths/bases/candidates are owner-authorized and version and existence conflict check/read-back verified.

Import operates per locale and never mutates source/another locale. It validates package/delivery/consent, current source/key/locale identities, stable IDs, placeholder/plural AST and owner. Target translation plus revision ledger commit atomically with exact owner/writer/base/candidate/rollback/version and existence conflict check. One locale conflict produces PARTIAL and cannot affect others. REVIEWED requires a distinct matching review receipt.
...
Affected key/locale revision set is incomplete and one vendor receipt missing. Freeze remains ACTIVE and source bytes unchanged. Complete request lifts only state, then catalog apply uses separate version and existence conflict check.
...
Evidence manifest references another build/source translation revision. Review rejects/stales exact receipts and passes no generic report path/status to gate.

### Case 10 — Export/import version and existence conflict check

Export package source revision changes before write: no package. During import target translation changes after preview: locale transaction writes neither translation nor ledger; new preview/authorization required.
...
A request declares TEAM_NARRATIVE source rows and a path/version-pinned v2 handoff.
...
payload identities, computing the payload with exactly `payload_revision` and
derived `handoff_id` omitted, and may proceed to the selected localize subcommand without
granting any extra write. Negative variants change one raw source revision, omit one
...
candidate whose producer, record ID, raw-manifest artifact revision, exact v2 extension
path/revision, COMPLETE coverage, unchanged conclusive native verdict, NONE
...
revalidate all request/manifest/catalog/package/per-key/locale/translation/freeze/
...
stale/expired scope, wrong extension or package declared revision, changed native verdict,
self-recording identity, reused target, non-created version and existence conflict check, or unknown read-back emit
no valid durable receipt and remain gate-ineligible.

### Case 14 — Package ID derivation is non-circular

Export validates the owner-assigned `package_id`, explicit `package_payload_revision`, and schema/path bindings without calculating identifiers from bytes; the package ID remains
`a stable LOCPKG business ID supplied by the package owner, serializes the completed
package, and separately revisions its raw file. A variant revision validation a placeholder or
completed `package_id`, omitting only the identifier field, or substituting the raw-file
revision is invalid and writes no package.
...
- gate handoff by bare report path without locale/build/source revisions;
- external transfer/recording without consent/contract/minimization;
- import/export without owner/base/candidate version and existence conflict check and read-back;
...
- TEAM_NARRATIVE source admission without the exact current persisted v2 handoff,
  equal authority, complete current revision sets, and LOCALIZATION_READY zero-blocker
  evidence.
- generic localization evidence without its exact current v2 extension, or a
  ...
  create-only targets, version and existence conflict check and verified read-back.
...
| LOC-007 | Sections D/J and Case 12 require explicit locales, bounded linked sources, and exact path/version-pinned narrative source admission |
...
| LOC-011 | Section G defines affected-key/locale revisions, vendor receipts, privacy/retranslation scope and immutable freeze lineage |
| LOC-012 | Section H bounds per-locale workers/time/delegation and enforces PARTIAL for missing responses |
| LOC-013 | Sections I/J and Cases 12/13 bind immutable downstream evidence and exact current upstream narrative revisions; neither a bare report, unpersisted generic candidate, nor conversation-only handoff is durable gate evidence |

## Output

Every result records subcommand, locale/page scope, exact identities that exist, parser/budget/omission counts, owner/consent/privacy gaps, writes pre/post revisions or READ_ONLY_NO_CHANGES, partial cursor/recovery state, one legal next action, and `auto_executed: false`.
