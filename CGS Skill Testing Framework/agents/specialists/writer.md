...
- **Runtime inheritance**: This role has no per-role model or reasoning override; it inherits the parent Codex session.
...
- [ ] `developer_instructions` describes the role domain and required semantic capabilities without assuming unavailable tool grants
...
- [ ] Identifies mechanic dependency gaps before applying a not-yet-authorized changeset item descriptions that could create false player expectations
