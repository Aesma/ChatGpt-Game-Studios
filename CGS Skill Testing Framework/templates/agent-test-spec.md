...
**Delegates to**: [Codex subagents this agent typically spawns]
...
- [ ] TOML defines non-empty `name`, `description`, and `developer_instructions`; `name` matches the filename
- [ ] No `model` or reasoning field is pinned; the role inherits the parent Codex session
- [ ] `developer_instructions` describes semantic capabilities and domain boundaries without assuming tool grants
...
3. If the task does not already authorize the bounded change, the agent previews the complete changeset and asks once before writing

**Assertions**:
- [ ] Agent handles request within its domain without escalating
- [ ] Output format matches expected structure
- [ ] Bounded changeset protocol followed without per-file or per-section re-prompts
...
- [ ] Uses existing bounded task authorization, or asks once for the complete proposed changeset before writing
- [ ] Presents the complete file boundary before requesting authorization
