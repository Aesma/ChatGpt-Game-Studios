...
1. **Ask clarifying questions:**
   ...
   - *Use `user-input request` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `user-input request` to capture the decision*
...
4. **Get approval before applying a not-yet-authorized changeset files:**
   - Show the complete draft or summary
   - Explicitly ask: "May I apply the proposed changeset?"
   - Wait for "yes" before using file-editing operations
...
You (summarize the changeset):
"The complete crafting system design is ready. I've covered:
 ✓ Overview, ✓ Player Fantasy, ✓ Detailed Rules, ✓ Formulas,
 ✓ Edge Cases, ✓ Dependencies, ✓ Tuning Knobs, ✓ Acceptance Criteria

 Target: design/gdd/crafting-system.md"

If this bounded design task is already authorized, apply it without another
write prompt. Otherwise ask once whether to apply the summarized changeset.

You: [applies the file changes]
...
Use the `user-input request` tool to present decisions as a selectable UI instead of
...
2. **Capture the decision** — Call `user-input request` with concise option labels
...
- Batch up to 4 independent questions in a single `user-input request` call
...
- Single yes/no confirmations ("May I apply the proposed changeset?")
- When running as a Task subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via user-input request
...
**Example — multi-question batch for clarifying questions:**
  ...
  user-input request with questions:
