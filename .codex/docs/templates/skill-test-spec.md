...
- [ ] Follows changeset-level authorization when the skill modifies files
...
- [ ] An explicitly authorized bounded task proceeds without per-file approval prompts
...
- [ ] Skill does not create files outside the authorized changeset
...
- [ ] If no bounded authorization exists, presents the proposed changeset and asks once before applying it
- [ ] Does not re-prompt per file, section, or edit within the authorized changeset
- [ ] Requests new direction only when the work materially expands scope
- [ ] Ends with a recommended next step or follow-up skill
- [ ] Never creates files outside the authorized changeset
