...
There are three organizational tiers. They describe responsibility and escalation,
not model selection. This repository defines no per-role model or reasoning
override; all 49 Codex subagents inherit the parent runtime.

- **Tier 1 — Directors**: make high-level decisions
...
- **Tier 2 — Department leads**: own their domain
  - `game-designer`, `lead-programmer`, `art-director`, `audio-director`,
    `narrative-director`, `qa-lead`, `release-manager`, `localization-lead`

- **Tier 3 — Specialists**: execute within their domain
...
6. **Prototype the mechanic** — Run `$prototype [core-mechanic]` (1–3 days, before detailed GDD authoring)
...
.codex/
  ...
  docs/
    ...
    settings-local-template.md     -- Personal config and hook-location guide
    templates/                     -- 40 recursively discovered document templates
```

The 11 path-scoped rule sets live in nested `AGENTS.md` files under their
effective directories; see `.codex/docs/rules-reference.md`.

## Changeset Authorization

If the user explicitly requests a bounded implementation, that request authorizes
the in-scope changeset. Codex should not ask again for each file or edit. For an
unapproved proposal, present the changeset and ask once; ask again only when work
materially expands scope or reaches a separately gated destructive/external action.
