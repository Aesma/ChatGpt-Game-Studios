...
- Incremental persistence inside one bounded changeset authorization
...
- Explicit "May I apply the proposed changeset?" before creating file
...
> In practice, agents now use the `user-input request` tool at decision points to
...
4. **Agents Get Approval Before Writing Files**
   - Explicit "May I apply the proposed changeset?" before using file-editing operations
...
3. Notice if the agent:
   ...
   - ✅ Requests approval before applying a not-yet-authorized changeset files
