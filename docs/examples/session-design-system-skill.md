...
>
> Proposed bounded changeset: create `design/gdd/movement-system.md` with the skeleton and fill every section accepted in this session. Apply it?
...
>
> Section 1 is accepted; I am continuing within the already-authorized document changeset.
...
*(These follow the same pattern: the user accepts or revises each design decision, and the agent persists accepted sections within the single authorized document changeset. Abbreviated here.)*
...
- **Incremental persistence within one authorization**: accepted sections are saved without repeated write prompts, so a crash loses at most one in-progress section
