...
**[Report a vulnerability →](https://github.com/Aesma/ChatGpt-Game-Studios/security/advisories/new)**
...
CGS is a **local development framework** — a trusted repository may load project
hooks and Codex subagents that run directly on your machine. Security issues are primarily about
contributed code that executes in users' environments without their awareness.

### High Severity
- Hooks (`.codex/hooks/*.sh` or `.codex/hooks.json`) that execute malicious or undisclosed shell
...
- Subagent definitions that escalate permissions or write outside an explicitly
  authorized bounded changeset
- Hook patterns that behave differently across platforms to conceal behavior
- Skills that write outside their documented scope or materially expand a
  changeset without renewed authorization

### Out of Scope
- The behavior of Codex or the Codex CLI itself
  (report through [OpenAI security](https://openai.com/security/))
...
- **Skills must not write outside their documented scope** or materially expand
  an authorized bounded changeset without renewed authorization
